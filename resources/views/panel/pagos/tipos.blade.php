...
<div class="flex items-start justify-between gap-4">
    <div>
        ...
        <p class="mt-1 panel-muted">Planes de pago (mensual, trimestral, temporada, etc.).</p>
    </div>
    ...
    <form class="mt-4 grid gap-3 lg:grid-cols-5"
        ...
        <div class="lg:col-span-2">
            ...
            <input
                ...
                placeholder="Mensual, Temporada, etc."
            >
        </div>
        ...
        <div>
            ...
            <input
                name="importe"
                value="{{ old('importe', $edit->importe ?? '') }}"
                class="panel-input w-full mt-1 px-4 py-3"
                placeholder="30.00"
            >
        </div>
        ...
        <div id="bloque_meses">
            ...
            <input
                name="duracion_meses"
                value="{{ old('duracion_meses', $edit->duracion_meses ?? 1) }}"
                class="panel-input w-full mt-1 px-4 py-3"
            >
        </div>
            ...
            <tbody>
                @forelse($tipos as $t)
                    <tr class="border-t panel-border">
                        ...
                        <td class="py-3">{{ $t->tipo_vigencia === 'temporada' ? 'Temporada' : 'Por meses' }}</td>
                        <td class="py-3">
                            {{ $t->tipo_vigencia === 'temporada' ? 'Hasta 30/06' : $t->duracion_meses . ' mes/es' }}
                        </td>
                        <td class="py-3">
...
<script>
    (function () {
        ...
        function refrescarFormularioTipoCuota() {
            const esTemporada = tipoVigencia.value === 'temporada';

            bloqueMeses.style.display = esTemporada ? 'none' : '';
            bloqueVentaInicio.style.display = esTemporada ? '' : 'none';
            bloqueVentaFin.style.display = esTemporada ? '' : 'none';
        }
